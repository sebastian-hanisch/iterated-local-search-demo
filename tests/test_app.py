...
def _run(ils_step=1, **state):
    at = AppTest.from_file(APP, default_timeout=300)
    for k, v in state.items():
        at.session_state[k] = v
    at.run()
    if ils_step != 1:                                                      # die App setzt den Schritt beim ersten Lauf zurück: erst danach wählen
        at.select_slider(key="ils_step").set_value(ils_step).run()
    return at
...
def test_default_run_has_no_exception_and_shows_the_measured_default():
    at = _run()
    _ok(at)
    assert _metric(at, "Beste Tour") == "0.1 %" and _metric(at, "Ein Hill-Climbing-Abstieg") == "6.8 %" and _metric(at, "HC-Neustarts (Kandidatenliste + DLB)") == "0.4 %"
...
def test_every_step_runs(step, n):
    at = _run(n_slider=n, budget_select=25000, ils_step=step)
    _ok(at)
    assert at.get("plotly_chart") and at.session_state["ils_step"] == step


def test_step_three_iteration_slider_and_play_kicks():
    at = _run(ils_step=3, budget_select=25000)
    _ok(at)
    lv = next(s for s in at.slider if s.key == "ils_iter")
    assert lv.value == lv.max
    lv.set_value(0).run()
    _ok(at)
    at2 = _run(ils_step=3, budget_select=25000)
    next(b for b in at2.button if b.label == "▶️ Kicks abspielen").click().run()
    _ok(at2)
...
def test_accept_toggle_runs_for_both_rules():
    for accept in C.ACCEPT_LABELS:
        _ok(_run(accept_select=accept, budget_select=25000))
...
@pytest.mark.parametrize("kw", [dict(n_slider=200, budget_select=50000), dict(n_slider=10, ballung_slider=100, budget_select=10000),
                                 dict(local_search_select="full", budget_select=25000), dict(n_bridges_slider=8, budget_select=10000),
                                 dict(accept_select="always", budget_select=25000), dict(start_radio="nearest", budget_select=25000)])
...
def test_permalink_values_are_clamped_and_snapped():
    ...
    assert at.session_state["n_bridges_slider"] == C.N_BRIDGES_MAX and at.session_state["budget_select"] == C.DEFAULT_BUDGET
...
def test_experiments_run_on_demand(monkeypatch):
    ...
    monkeypatch.setitem(ev.SWEEP_VALUES, "n_bridges", (1, 2))
    ...
    at = _run(n_slider=10, budget_select=10000)
    for key, flag in (("budget_start", "budget_on"), ("bridges_start", "bridges_on"), ("accept_start", "accept_on"), ("ls_start", "ls_on"),
...
def test_footer_and_grenzen_are_present():
    ...
    assert any("Die lokale Suche ist billig" in m.value for m in at.markdown)

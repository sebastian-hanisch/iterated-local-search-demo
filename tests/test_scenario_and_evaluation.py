...
Neustarts (beide Varianten), Sweeps, Streuung)."""
...
def test_instance_is_deterministic_seed_dependent_and_matches_the_frozen_hill_climbing_instance():
    ...
    # eingefroren: dieselbe Instanz wie in der Hill-Climbing-/Simulated-Annealing-Demo (Seed 100000, 60 Stopps, gleichverteilt)
...
def test_without_hill_climbing_the_comparison_fields_are_empty_and_fast():
    ...
    assert a.hc is None and a.hcr_tour is None and a.hcr_starts == 0 and a.dlbr_starts == 0
...
def test_dlb_restarts_uses_at_least_one_descent_and_is_far_cheaper_than_a_full_rescan_restart():
    ...
    assert starts_dlb > 20 * starts_hcr                          # gemessen: viel mehr Starts bei gleichem Budget
...
def _fake(gap, dlbr_gap):
    class F:
        pass
    f = F()
    f.gap, f.dlbr_gap = gap, dlbr_gap
    return f
...
def test_verdict_of_a_real_run_at_the_default_settings():
    assert ev.verdict(ev.analyse(ev.Settings())) in ("beats_hc", "comparable")
...
def test_sweep_values_labels_and_ordering():
    ...
    assert [r["value"] for r in rows] == list(ev.ILS.LOCAL_SEARCHES) and rows[0]["gap"] < rows[1]["gap"]      # DLB besser als voller Rescan
...
def test_chain_spread_returns_one_value_per_chain_and_is_deterministic():
    ...
    assert len(a["ils"]) == len(a["hc"]) == 5 and np.array_equal(a["ils"], b["ils"]) and np.array_equal(a["hc"], b["hc"])
